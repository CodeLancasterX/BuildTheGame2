...
public class GamePanel extends JPanel implements GameObserver, MouseListener, MouseMotionListener {
	...
	private static final Color COLOR_PURPLE = new Color(122, 130, 207);
	private static final Color COLOR_ORANGE = new Color(244, 199, 62);
	private static final Color COLOR_DARK_ORANGE = new Color(219, 140, 69);
	private static final Color COLOR_GREEN = new Color(105, 219, 126);
	private static final Color COLOR_DARK_PURPLE = new Color(211, 82, 122);
	private static final Color COLOR_LIGHT_GREEN = new Color(157, 235, 81);
	private static final Color COLOR_DARK_RED = new Color(199, 82, 79);
	private static final Color COLOR_LIGHT_BLUE = new Color(104, 193, 224);
	private static final Color COLOR_EMERALD_GREEN = new Color(97, 229, 172);
		...
		for (int i = 0; i < blocks.length; i++) {
			for (int j = 0; j < blocks.length; j++) {
				if (blocks[j][i]) {
					...
					boolean isSelected;
					if (selectedPuzzleBlockId > -1) isSelected = puzzleBlock == puzzleBlocks[selectedPuzzleBlockId];
					else isSelected = false;

					int x1;
					if (isSelected) if (isSelected) x1 = this.x + i * TILE_SIZE + i * TILE_GAP;
					else x1 = x + i * TILE_SIZE + i * TILE_GAP;
					else if (isSelected) x1 = this.x + i * TILE_MINI_SIZE + i * TILE_GAP;
					else x1 = x + i * TILE_MINI_SIZE + i * TILE_GAP;
					int y1;
					if (isSelected) if (isSelected) y1 = this.y + j * TILE_SIZE + j * TILE_GAP;
					else y1 = this.y + j * TILE_MINI_SIZE + j * TILE_GAP;
					else if (isSelected) y1 = y + j * TILE_SIZE + j * TILE_GAP;
					else y1 = y + j * TILE_MINI_SIZE + j * TILE_GAP;
